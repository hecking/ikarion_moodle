...
//neu
function groups_get_user($userid, $courseid){
    global $DB;
    ...
    $sql = "SELECT u.id, $allusernamefields
              FROM mdl_course c
...
function groups_get_attributes($userid, $courseid, $attributes = array()){
    global $DB;
    ...
    $params['courseid4'] = $courseid;

    if(!empty($attributes)){

        if($attributes['lang'] != 'disabled'){
            $attributefields .= "u.lang, ";
        }

        if($attributes['country'] != 'disabled'){
            $attributefields .= "u.country, ";
        }

        if($attributes['grades'] != 'disabled'){
            $attributefields .= "q.grades, ";

            $grades = "LEFT JOIN
               (SELECT AVG(g.grade) AS grades, g.userid
                  FROM mdl_quiz_grades g, mdl_quiz q
                 WHERE g.quiz = q.id
                   AND q.course = :courseid1
              GROUP BY userid) q
                    ON u.id = q.userid
                    ";
        ...
        if($attributes['assignments'] != 'disabled'){
            $attributefields .= "a.assignments, ";

            $assignments = "LEFT JOIN
            (SELECT AVG(g.grade) AS assignments, g.userid
               FROM mdl_assign a, mdl_assign_grades g
              WHERE g.assignment = a.id
                AND a.course = :courseid2
           GROUP BY g.userid) a
                 ON u.id = a.userid
                 ";
        ...
        if($attributes['forum'] != 'disabled'){
            $attributefields .= "f.forum, ";

            $forum = "LEFT JOIN
               (SELECT COUNT(p.id) AS forum, p.userid
        ...
        if($attributes['activity'] != 'disabled'){
            $attributefields .= "l.activity, ";

            $activity = "LEFT JOIN
                (SELECT COUNT(*) AS activity, userid
    ...
    if(empty($attributefields)){
        ...
        $attributefields = substr($attributefields, 0, -2);
        $sql = "SELECT u.id, $attributefields
                  FROM mdl_user u
                 $join
                 WHERE u.id = :userid";
    }
