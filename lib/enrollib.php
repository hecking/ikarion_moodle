...
/*
 * New!
 * Get experimental conditions for current course
 * @param int $courseid
 * @return array $roles_array OR false
 */
function enrol_get_roles_of_conditions($courseid){

    global $DB;

    $roleids = $DB->get_record_sql('SELECT roleid FROM {enrol} WHERE courseid = '.$courseid.' AND LENGTH( roleid ) >2');

    if($roleids && $roleids->roleid) {
        $roles_array = explode(',', $roleids->roleid);
        return $roles_array;
    }else{
        return false;
    }
