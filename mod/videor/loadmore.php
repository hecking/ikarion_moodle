...
$videor = $DB->get_record('videos', array('id' => $_GET['id']), '*', MUST_EXIST);

$videos = $DB->get_records('videor', array('videoid' => $_GET['id']));
$video = reset($videos);
...
foreach ($result as $value) {
    ...
    foreach ($similar as $value2) {
        ...
        $cm = get_coursemodule_from_instance('videor', $cvideo->id, $cvideo->course, false, MUST_EXIST);
        $data->course = $courseid;
        $data->vid = $cm->id;
