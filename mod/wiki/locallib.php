...
function wiki_save_page($wikipage, $newcontent, $userid) {
    ...
    if (has_capability('mod/wiki:editpage', $context)) {
        ...
        //TODO this is where a new conceptnetwork is saved.
        $contentwithouthtml = html_entity_decode(rip_tags($newcontent));
        $concepts = mod_wiki_text2network_webservice($contentwithouthtml);

        $version = wiki_get_current_version($wikipage->id);

        $concepts = json_encode(mod_wiki_create_conceptnetwork_with_userid(json_decode($version->concepts), json_decode($concepts), $userid), JSON_UNESCAPED_UNICODE);



        $version->content = $newcontent;
        $version->concepts = $concepts;
...
/**
 * Call the text2network webservice to convert a text into a conceptnetwork.
 *
 * @param string $text
 * @param string $lang
 ...
 * @return mixed|null
 */
function mod_wiki_text2network_webservice($text = false, $lang = "de", $window_size = 4, $doc_id = "test") {

    $text = rawurlencode($text);

    $serviceUrl = "http://textanalytics.collide.info:8080/textanalytics-jersey-1.0-SNAPSHOT/text2network";
    $data = "text=$text&lang=$lang&window_size=$window_size&doc_id=$doc_id";
...
/**
 * Builds the difference of two conceptnetworks. A-B.
 *
 * @param mixed $conceptnetworkA
 * @param mixed $conceptnetworkB
 * @return mixed
 */
function mod_wiki_diff_conceptnetwork($conceptnetworkA, $conceptnetworkB)
{

    if (!isset($conceptnetworkA->data))
        return $conceptnetworkA;
    elseif (!isset($conceptnetworkB->data))
        return $conceptnetworkA;
    ...
    for ($x = 0; $x < sizeof($conceptnetworkA->data->nodes); $x++) {

        for ($y = 0; $y < sizeof($conceptnetworkB->data->nodes); $y++) {


            if (strcmp($conceptnetworkA->data->nodes[$x]->id, $conceptnetworkB->data->nodes[$y]->id) == 0) {
        ...
        if ($in == false) {
            ...
            $temp->data->nodes[$z] = $conceptnetworkA->data->nodes[$x];
            $z++;
    ...
    for ($x = 0; $x < sizeof($conceptnetworkA->data->edges); $x++) {

        for ($y = 0; $y < sizeof($conceptnetworkB->data->edges); $y++) {


            if (((strcmp($conceptnetworkA->data->edges[$x]->source, $conceptnetworkB->data->edges[$y]->source) == 0) and (strcmp($conceptnetworkA->data->edges[$x]->target, $conceptnetworkB->data->edges[$y]->target) == 0)) or ((strcmp($conceptnetworkA->data->edges[$x]->source, $conceptnetworkB->data->edges[$y]->target) == 0) and (strcmp($conceptnetworkA->data->edges[$x]->target, $conceptnetworkB->data->edges[$y]->source) == 0))) {
        ...
        if ($in == false) {
            ...
            $temp->data->edges[$z] = $conceptnetworkA->data->edges[$x];
            $temp->data->edges[$z]->id = $z+1;
            $z++;
...
/**
 * Builds intersection of two conceptnetworks.
 *
 * @param $conceptnetworkA
 * @param $conceptnetworkB
 * @return stdClass
 */
function mod_wiki_intersect_conceptnetwork($conceptnetworkA, $conceptnetworkB) {



    $temp = new stdClass();

    if (isset($conceptnetworkA))
        $temp->metadata = $conceptnetworkA->metadata;
    elseif (isset($conceptnetworkB))
        $temp->metadata = $conceptnetworkB->metadata;
    $in = false;


    $z = 0;

    //Makes the intersect of the nodes.
    for ($x = 0; $x < sizeof($conceptnetworkA->data->nodes); $x++) {
    ...
    //Makes the intersect of the edges.
    for ($x = 0; $x < sizeof($conceptnetworkA->data->edges); $x++) {
...
/**
 * Merges two conceptnetworks begining with A.
 *
 * @param mixed $conceptnetworkA
 * @param mixed $conceptnetworkB
 * @return mixed
 */
function mod_wiki_merge_conceptnetwork($conceptnetworkA, $conceptnetworkB)
{

    if (!isset($conceptnetworkA->data->nodes) && isset($conceptnetworkB->data->nodes))
        $conceptnetworkA->data->nodes = $conceptnetworkB->data->nodes;
    elseif (isset($conceptnetworkA->data->nodes) && isset($conceptnetworkB->data->nodes)) {



        $startnodes = sizeof($conceptnetworkA->data->nodes);
        $endnodes = sizeof($conceptnetworkA->data->nodes) + sizeof($conceptnetworkB->data->nodes);

        $y = 0;

        //merges the nodes.
        for ($x = $startnodes; $x < $endnodes; $x++) {


            $conceptnetworkA->data->nodes[$x] = $conceptnetworkB->data->nodes[$y];
    ...
    if (!isset($conceptnetworkA->data->edges) && isset($conceptnetworkB->data->edges))
        $conceptnetworkA->data->edges = $conceptnetworkB->data->edges;
    elseif (isset($conceptnetworkA->data->edges) && isset($conceptnetworkB->data->edges)) {

        $startedges = sizeof($conceptnetworkA->data->edges);
        $endedges = sizeof($conceptnetworkA->data->edges) + sizeof($conceptnetworkB->data->edges);
        $y = 0;


        //merges the edges.
        for ($x = $startedges; $x < $endedges; $x++) {

            $conceptnetworkA->data->edges[$x] = $conceptnetworkB->data->edges[$y];
            $conceptnetworkA->data->edges[$x]->id = $x + 1;
...
/**
 * Builds the union of two conceptnetworks.
 *
 * @param mixed $conceptnetworkA
 * @param mixed $conceptnetworkB
 * @return mixed
 */
function mod_wiki_union_conceptnetwork($conceptnetworkA, $conceptnetworkB) {

    $diffBA = mod_wiki_diff_conceptnetwork($conceptnetworkB, $conceptnetworkA);
    $conceptnetworkA = mod_wiki_merge_conceptnetwork($conceptnetworkA, $diffBA);
...
/**
 * Set attribute userid for conceptnetwork.
 *
 * @param $conceptnetwork
 * @param $userid
 * @return mixed
 */
function mod_wiki_save_userid_in_conceptnetwork($conceptnetwork, $userid) {

    //Sets userid attribute for nodes.
    for ($x = 0; $x < sizeof($conceptnetwork->data->nodes); $x++) {

        $conceptnetwork->data->nodes[$x]->userid = $userid;

    }

    //Sets userid attribute for edges.
    for ($y = 0; $y < sizeof($conceptnetwork->data->edges); $y++) {

        $conceptnetwork->data->edges[$y]->userid = $userid;
...
/**
 * Create a new conceptnetwork of $conceptnetworkB, where all similar nodes and edges of $conceptnetworkA get the userid from it.
 * All nodes and egdes from $conceptnetworkB, which don't exist in $conceptnetworkA get the attribute userid = $userid.
 *
 * @param $conceptnetworkA
 * @param $conceptnetworkB
 * @param $userid
 ...
 */
function mod_wiki_create_conceptnetwork_with_userid($conceptnetworkA, $conceptnetworkB, $userid){
    $diff = new stdClass();
    $temp = new stdClass();


    if (!isset($conceptnetworkB->data->nodes) and !isset($conceptnetworkA))
        return null;
    elseif (!isset($conceptnetworkA))
        return mod_wiki_save_userid_in_conceptnetwork($conceptnetworkB, $userid);
    elseif (!isset($conceptnetworkB->data->nodes))
        return $conceptnetworkA;



    $diff = mod_wiki_diff_conceptnetwork($conceptnetworkB, $conceptnetworkA);
    $temp = mod_wiki_intersect_conceptnetwork($conceptnetworkA, $conceptnetworkB);

    if (!isset($diff->data) and !isset($temp->data))
        return null;
    elseif (!isset($diff->data))
        return $temp;
    elseif (!isset($temp->data))
        return mod_wiki_save_userid_in_conceptnetwork($diff, $userid);


    $temp = mod_wiki_merge_conceptnetwork($temp, mod_wiki_save_userid_in_conceptnetwork($diff, $userid));
...
/**
 * Set a color attribute for all nodes and edges of a conceptnetwork.
 *
 * @param $conceptnetwork
 * @param $color
 * @return mixed
 */
function mod_wiki_color_conceptnetwork($conceptnetwork, $color){

    //Colors the nodes.
    for ($x= 0; $x < sizeof($conceptnetwork->data->nodes); $x++ ) {

        $conceptnetwork->data->nodes[$x]->color = $color;
    }

    //Colors the edges.
    for ($x= 0; $x < sizeof($conceptnetwork->data->edges); $x++ ) {

        $conceptnetwork->data->edges[$x]->color = $color;
    }
...
/**
 * Create a conceptnetwork, which shows alls differences of two conceptnetworks from different versions.
 * Similar nodes and edges of both networks get the color attribute blue, new nodes and edges of the newer version get green
 * and nodes and edges, which only occur in the old version but not in the new get the color attribute red.
 *
 * @param $conceptnetworknew
 * @param $conceptnetworkold
 * @return mixed|stdClass
 */
function mod_wiki_color_diff_versions ($conceptnetworknew, $conceptnetworkold) {


    $blue = mod_wiki_intersect_conceptnetwork($conceptnetworknew, $conceptnetworkold);

    $red = mod_wiki_diff_conceptnetwork($conceptnetworkold, $conceptnetworknew);

    $green = mod_wiki_diff_conceptnetwork($conceptnetworknew, $conceptnetworkold);

    //Colors the single parts of the new network.
    if (isset($blue->data->nodes))
        mod_wiki_color_conceptnetwork($blue, "CornflowerBlue");

    if (isset($red->data->nodes))
        mod_wiki_color_conceptnetwork($red, "red");

    if (isset($green->data->nodes))
        mod_wiki_color_conceptnetwork($green, "LightGreen");

    //Checks which parts are empty and then returns the merged network.
    if (isset($blue->data->nodes) and isset($red->data->nodes) and isset($green->data->nodes)) {

        return mod_wiki_union_conceptnetwork(mod_wiki_union_conceptnetwork($blue, $green), $red);

    }elseif (isset($blue->data->nodes) and !isset($red->data->nodes) and !isset($green->data->nodes)) {

        return $blue;

    }elseif (isset($blue->data->nodes) and !isset($red->data->nodes) and isset($green->data->nodes)) {

        return mod_wiki_union_conceptnetwork($blue, $green);
        ...
        return mod_wiki_union_conceptnetwork($blue, $red);

    }elseif (!isset($blue->data->nodes) and isset($red->data->nodes) and isset($green->data->nodes)) {

        return mod_wiki_union_conceptnetwork($green, $red);
...
/**
 * Create the supergraph from an array of concetnetworks.
 *
 * @param $conceptnetworkarray
 * @return mixed|stdClass
 */
function mod_wiki_create_supergraph($conceptnetworkarray) {

    $temp = new stdClass();

    foreach ($conceptnetworkarray as $conceptnetwork) {


        $temp = mod_wiki_union_conceptnetwork($temp, $conceptnetwork);
...
/**
 * Get the versions of a wiki by pageid.
 *
 * @param $pageid
 * @return array
 */
function mod_wiki_get_versions_conceptnetwork($pageid) {

    $wikiversions = wiki_get_wiki_page_versions($pageid, 0, 1000);
    $conceptnetworks = [];

    //Saves all conceptnetworks of the versions in an array.
    foreach ($wikiversions as $versions) {

        $concepts = json_decode($versions->concepts);
